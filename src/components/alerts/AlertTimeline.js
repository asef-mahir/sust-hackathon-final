...
export function AlertTimeline({ timeline }) {
  return (
    <Card className="bg-surface border-white/10">
      <CardHeader>
        <CardTitle className="text-sm font-medium text-white/80">Audit Trail</CardTitle>
      </CardHeader>
      <CardContent>
        {timeline.length === 0 ? (
          <EmptyState title="No actions taken on this alert yet." />
        ) : (
          <ol className="flex flex-col gap-4 border-l border-white/10 pl-4">
            {timeline.map((event) => (
              <li key={event.id} className="relative">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs text-white/30">({event.actor?.role})</span>
                  {event.toStatus ? <StatusPill status={event.toStatus} /> : null}
                </div>
                {event.note ? (
                  <p className="mt-1 text-sm text-white/60">{event.note}</p>
                ) : null}
